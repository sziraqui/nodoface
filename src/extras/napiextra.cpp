...
Napi::Boolean NapiExtra::toNapi(Napi::Env env, bool val)
{
    return Napi::Boolean::New(env, val);
}

Napi::Number NapiExtra::toNapi(Napi::Env env, int val)
{
    return Napi::Number::New(env, val);
}

Napi::Number NapiExtra::toNapi(Napi::Env env, double val)
{
    return Napi::Number::New(env, val);
}

Napi::String NapiExtra::toNapi(Napi::Env env, std::string &val)
{
    return Napi::String::New(env, val);
}

template <typename anyType>
Napi::Array NapiExtra::toNapiArray(Napi::Env env, std::vector<anyType> &vec)
{
...
Napi::Object NapiExtra::cv2Napi(Napi::Env env, cv::Point_<numericType> &point)
{
...
template <class numericType>
Napi::Object NapiExtra::cv2Napi(Napi::Env env, cv::Point3_<numericType> point)
{
    auto obj = Napi::Object::New(env);
    obj["x"] = Napi::Number::New(env, point.x);
    obj["y"] = Napi::Number::New(env, point.y);
...
template <class numericType>
Napi::Object NapiExtra::cv2Napi(Napi::Env env, cv::Rect_<numericType> &rect)
{
    auto obj = Napi::Object::New(env);
...
template <typename anyType>
Napi::Array NapiExtra::cv2NapiArray(Napi::Env env, std::vector<anyType> &vec)
{
    Napi::Array array = Napi::Array::New(env, vec.size());
    for (uint i = 0; i < vec.size(); ++i)
    {
        array[i] = NapiExtra::cv2Napi(env, vec[i]);
    }
    return array;
}
template <class numericType>
cv::Rect_<numericType> NapiExtra::Napi2Rect(Napi::Object rectObj)
{
...
std::vector<cv::Rect> NapiExtra::Napi2RectVector(Napi::Array arr)
{
    std::vector<cv::Rect> vec(arr.Length());
    for (uint i = 0; i < vec.size(); ++i)
    {
        vec[i] = NapiExtra::Napi2Rect(arr.Get(i).As<Napi::Object>());
    }
    return vec;
}

cv::Scalar NapiExtra::Napi2ScalarBgr(Napi::Object rgbColor)
{
...
std::vector<std::string> NapiExtra::Napi2StringVector(const Napi::Array arr)
{
...
template <class numericType>
cv::Point_<numericType> NapiExtra::Napi2Point(Napi::Object obj)
{
...
template <class numericType>
cv::Point3_<numericType> NapiExtra::Napi2Point3(Napi::Object obj)
{
    numericType x = (numericType)obj.Get("x").As<Napi::Number>().DoubleValue();
    numericType y = (numericType)obj.Get("y").As<Napi::Number>().DoubleValue();
...
Napi::Object NapiExtra::detectionResult2Napi(Napi::Env env, std::vector<cv::Rect_<float>> &out_bboxes, std::vector<float> &out_confidences)
{
...
template <class numericType>
Napi::TypedArrayOf<numericType> NapiExtra::cvMat2TypedArray(Napi::Env env, cv::Mat_<numericType> &mat)
{
...
template <class numericType>
Napi::TypedArrayOf<numericType> NapiExtra::cvMat2TypedArray(Napi::Env env, cv::Mat &mat)
{
    size_t length = mat.total() * mat.channels();
    Napi::ArrayBuffer ab = Napi::ArrayBuffer::New(env, mat.data, sizeof(numericType) * length);
    Napi::TypedArrayOf<numericType> arr = Napi::TypedArrayOf<numericType>::New(env, length, ab, 0);
